...
function Hero() {
  ...
  console.log(blogs)
  return (
    <div className="container mx-auto my-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 p-6">
      {blogs && blogs.length > 0 ? (
        blogs.slice(0, 5).map((element, idx) => {
         return <Link to ={'/'} key={element._id}
         className="bg-white rounded-lg hover:shadow-lg overflow-hidden transform hover:scale-105 transition-transform-300"
         >
